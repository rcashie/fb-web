...
class NewMovePage extends PolymerElement {
    static get template() {
        return html`
            ...
            <article hidden="[[_hiddenSections.content]]">
                ...
                <attribute-set-editor
                    id="charAttributes"
                    title-placeholder="e.g. Damage"
                    value-placeholder="e.g. 500"
    ...
    connectedCallback() {
        ...
        // TODO: Make this string localizable
        document.title = 'New move proposal | framebastard';
    }
    ...
    _onQueryParamsChanged(newParams) {
        ...
        if (target) {
            ...
            ajax.url = `/doc-api/v1/docs/moves/${decodeURI(target)}`;
            ajax.generateRequest();
            section = 'stencil';
        } else {
            this._charId = decodeURI(newParams.char);
            section = 'content';
        }
    ...
    _getCancelSignInUrl(queryParams) {
        ...
        return target
            ? `/docs/moves/${target}`
            : `/docs/chars/${queryParams.char}`;
    }

    _getIdPrefix(queryParams) {
        const charId = queryParams.char;
    ...
    _onSubmitClicked() {
        if (!this.$.moveId.valid
            || !this.$.moveTitle.valid
    ...
    _onPostError(event) {
        ...
        this._errorMessage = event.detail.request.status === 401
            ? 'Darn, you\'re no longer logged in. Please login.'
...
customElements.define(NewMovePage.is, NewMovePage);
